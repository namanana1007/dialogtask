...
class DialogManager:
    ...
    def initialize_episode(self, command):
        ...
        self.user_action = self.user.initialize_episode(command)
    ...
    def next_turn_qian(self, record_training_data=True):
        ...
        self.episode_over, dialog_status = self.user.end_or_not(self.sys_action)
        return self.episode_over,self.agent_action['act_slot_response']['nl']

    def next_turn_hou(self, command, record_training_data=True):   
        ########################################################################
        #   CALL USER TO TAKE HER TURN
        ########################################################################
        self.sys_action = self.state_tracker.dialog_history_dictionaries()[-1]
        self.user_action, self.episode_over, dialog_status = self.user.next(command, self.sys_action)
